...
export default function PracticeBookingScreen() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createBooking({ nurseId: "practice-nurse-id", date });
      setStatus("waiting");
      setDate("");
    } catch (err: any) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-100 p-10">
        <div className="max-w-md w-full text-center">
          <h1 className="text-2xl font-bold text-slate-800 mb-6">Practice Booking Screen</h1>
          <div className="bg-white border border-slate-100 rounded-2xl p-8 shadow-xl shadow-blue-500/5">
          ...
          <div className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-1.5">
                ...
                <label
                  htmlFor="practice-appointment"
                  className="text-[11px] font-bold uppercase tracking-wider text-slate-400 ml-1"
                >
                  Appointment Details
                </label>
                <div className="relative group/input">
                  <input
                    id="practice-appointment"
                    type="datetime-local"
                    value={date}
              ...
              >
                {loading ? (
                  <Loader2 data-testid="loader" className="w-5 h-5 animate-spin text-white/70" />
                ) : (
